...
class ReviewWorkspaceBridge:
    def __init__(self, workspace_dir, reviewer="Unknown Reviewer"):
        self.workspace_dir = Path(workspace_dir)
        self.reviewer = reviewer
        self.workspace_dir.mkdir(parents=True, exist_ok=True)

    @property
    def draft_pack_dir(self): return self.workspace_dir / "draft_pack"
    @property
    def review_session_path(self): return self.workspace_dir / "review_session.json"
    @property
    def promoted_pack_dir(self): return self.workspace_dir / "promoted_pack"
    ...
    def save_session(self, session): export_review_state_json(session, self.review_session_path)
